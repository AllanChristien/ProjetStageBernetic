...
$recherche = mysqli_real_escape_string(htmlspecialchars('intranetbe', $_POST['recherche'])); //sécurisation des variables
$mode = mysqli_real_escape_string(htmlspecialchars($_POST['mode']));
...
$selection_recherche = mysqli_query("SELECT * FROM documents
WHERE ($valeur_requete1 OR $valeur_requete2 OR $valeur_requete3) ORDER BY id LIMIT 0,100 ") or die(mysqli_error()); //requete contenant le résultat de la boucle
}
$nombre_resultats = mysqli_num_rows($selection_recherche); //comptage du nombre d'entrées sélectionnées par la recherche
if ($nombre_resultats == 0) //s'il n'y a pas de résultat
{ ?>
<br /><br />

<div align = "center" class ="soustitre">Votre recherche n'a donné aucun résultat.<br />
...
<table width="1000" class="cadres" border="0" cellpadding="0" cellspacing="0" bordercolor="#333333">
  <tr>
    <td align="center" valign="middle"><table width="60%" border="0" cellpadding="0" cellspacing="0">
      <tbody>
        <tr><br>
          <td height="30" align="center" valign="middle" bgcolor="#FFFFFF" class="TextPlanNoir1"> Vous avez recherché : <strong><?php echo $recherche ; ?></strong> - 
        <?php echo '(Nombre de résultats :<strong>';?>
      <?php  echo $nombre_resultats."</strong>)" ; //nombre de résultats ?>
            <?php if ($nombre_resultats == 100) { echo "(Affichage limité; à 100 lignes) <br><br> "  ; }  ?>
            </td>
          </tr>
        <tr>
...
<table width="1064" border="0" cellspacing="3" cellpadding="0">
  <tr>
    ...
    <td width="77" height="30" align="center" valign="middle" bgcolor="#BBB8B8"><span class="TextPlanNoir1">Numéro du document</span><br /></td>
    <td width="176" height="30"  align="center" valign="middle" bgcolor="#BBB8B8" class="TextPlanNoir1">Créé par</td>
...
<br />
<?php 
while($resultats = mysqli_fetch_array($selection_recherche) ) //boucle affichant les résultats
{
$ID = $resultats["id"] ;     
$NomPosteur = $resultats['posteur'] ;
$DateJour = $resultats['dateproduction'] ;
